...
class TopicQueryService:
    ...
    def query_topic_posts(
        ...
        image_map = self._load_image_paths(resolved_topic_id, [row['post_number'] for row in rows]) if include_images else {}
        items = [
            QueryPostItem(
                post_id=row['post_id'],
        ...
        return QueryResult(topic_id=resolved_topic_id, total_hits=total_hits, items=items)
    ...
    def _fetch_rows(
        ...
        if keyword:
            try:
                rows, total = self._fts_query(topic_id, keyword, author, op_username, date_from, date_to, has_images, limit, offset, order_sql)
                if rows:
                    return rows, total
            except sqlite3.OperationalError:
                pass
            return self._like_query(topic_id, keyword, author, op_username, date_from, date_to, has_images, limit, offset, order_sql)
        return self._plain_query(topic_id, author, op_username, date_from, date_to, has_images, limit, offset, order_sql)

    def _base_filters(self, topic_id: int, author: str | None, op_username: str | None, date_from: str | None, date_to: str | None, has_images: bool | None) -> tuple[list[str], list]:
        clauses = ['p.topic_id = ?']
    ...
    def _plain_query(self, topic_id: int, author: str | None, op_username: str | None, date_from: str | None, date_to: str | None, has_images: bool | None, limit: int | None, offset: int, order_sql: str) -> tuple[list[sqlite3.Row], int]:
        clauses, params = self._base_filters(topic_id, author, op_username, date_from, date_to, has_images)
        where_sql = ' AND '.join(clauses)
        total = self.sqlite_store.conn.execute(f'SELECT COUNT(*) FROM posts p WHERE {where_sql}', params).fetchone()[0]
    ...
    def _like_query(self, topic_id: int, keyword: str, author: str | None, op_username: str | None, date_from: str | None, date_to: str | None, has_images: bool | None, limit: int | None, offset: int, order_sql: str) -> tuple[list[sqlite3.Row], int]:
        clauses, params = self._base_filters(topic_id, author, op_username, date_from, date_to, has_images)
        clauses.append('(COALESCE(p.plain_text, "") LIKE ? OR COALESCE(p.raw_markdown, "") LIKE ? OR COALESCE(p.username, "") LIKE ?)')
        like_term = f'%{keyword}%'
    ...
    def _fts_query(self, topic_id: int, keyword: str, author: str | None, op_username: str | None, date_from: str | None, date_to: str | None, has_images: bool | None, limit: int | None, offset: int, order_sql: str) -> tuple[list[sqlite3.Row], int]:
        clauses, params = self._base_filters(topic_id, author, op_username, date_from, date_to, has_images)
        where_sql = ' AND '.join(clauses)
        total_sql = f'''
            SELECT COUNT(*)
    ...
    def _load_image_paths(self, topic_id: int, post_numbers: Iterable[int], limit_per_post: int = 3) -> dict[int, list[str]]:
        post_numbers = [post_number for post_number in post_numbers if post_number is not None]
        if not post_numbers:
            return {}
