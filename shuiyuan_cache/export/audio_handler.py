...
AUDIO_RAW_PATTERN = re.compile(r'\[.*?\|audio\]\(upload://([a-zA-Z0-9]+)\.([a-zA-Z0-9]+)\)')
...
def _extract_audio_urls(cooked_content: str) -> list[str]:
    ...
    return [source['src'] for audio_tag in audio_tags for source in audio_tag.find_all('source') if source.get('src')]
...
def _collect_audio_links(topic: str | int) -> list[tuple[str, str]]:
    ...
    for post in cache_bridge.iter_json_posts(topic_id):
        ...
        raw_match = [f'{sha1}.{ext}' for sha1, ext in AUDIO_RAW_PATTERN.findall(raw_content)]
