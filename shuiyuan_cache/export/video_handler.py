...
VIDEO_RAW_PATTERN = re.compile(r'\[.*?\|video\]\(upload://([a-zA-Z0-9]+)\.([a-zA-Z0-9]+)\)')
VIDEO_COOKED_PATTERN = re.compile(r'class="video-placeholder-container" data-video-src="([^"]+)"')
...
def _collect_video_links(topic: str | int) -> list[tuple[str, str]]:
    ...
    for post in cache_bridge.iter_json_posts(topic_id):
        ...
        raw_match = [f'{sha1}.{ext}' for sha1, ext in VIDEO_RAW_PATTERN.findall(raw_content)]
