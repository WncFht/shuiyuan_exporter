...
class BrowserAuthManager:
    ...
    def setup_interactive(
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                return self._save_auth_artifacts(
                    context, browser=browser, login_url=resolved_login_url
                )
            finally:
                context.close()

    def refresh_from_profile(
        self, browser: str = "edge", headless: bool = False
    ) -> AuthSetupResult:
        self.config.auth_root.mkdir(parents=True, exist_ok=True)
        self.config.browser_profile_dir.mkdir(parents=True, exist_ok=True)
        browser_type_name, channel = self._resolve_browser(browser)
        with sync_playwright() as playwright:
            ...
            try:
                ...
                return self._save_auth_artifacts(
                    context,
                    browser=browser,
                    login_url=f"{self.config.base_url.rstrip('/')}/latest",
                )
            finally:
                context.close()
    ...
    def auth_status(self, check_live: bool = False) -> dict:
        ...
        cookie_text = (
            self.config.cookie_path.read_text(encoding="utf-8").strip()
            if cookie_exists
            else ""
        )
        payload = {
            ...
            "storage_state_cookie_count": count_domain_cookies(
    ...
    def _save_auth_artifacts(
        self, context: BrowserContext, browser: str, login_url: str
    ) -> AuthSetupResult:
        ...
        filtered_cookies = [
            ...
            if self._domain_matches(cookie.get("domain", ""), base_host)
        ]
    ...
    def _touch_forum_home(self, page) -> None:
        try:
            page.goto(
                f"{self.config.base_url.rstrip('/')}/latest",
    ...
    def _domain_matches(cookie_domain: str, host: str) -> bool:
        ...
        return normalized_domain == normalized_host or normalized_host.endswith(
            f".{normalized_domain}"
        )

    @staticmethod
    def _print_setup_instructions(
        browser: str, profile_dir: Path, storage_state_path: Path
    ) -> None:
        ...
        print(
            "建议操作：先访问 https://shuiyuan.sjtu.edu.cn/latest ，确认能正常看到已登录页面。"
        )
