...
ATTACHMENT_RAW_PATTERN = re.compile(r'\[.*?\|attachment\]\(upload://([a-zA-Z0-9]+)\.([a-zA-Z0-9]+)\)')
ATTACHMENT_COOKED_PATTERN = re.compile(r'class="attachment" href="([^"]+)"')
...
def _collect_attachment_links(topic: str | int) -> list[tuple[str, str]]:
    ...
    for post in cache_bridge.iter_json_posts(topic_id):
        ...
        cooked_match = ATTACHMENT_COOKED_PATTERN.findall(cooked_content)
        if not cooked_match:
            continue
        raw_content = cache_bridge.get_post_raw(topic_id, int(post['post_number']))
        raw_match = [f'{sha1}.{ext}' for sha1, ext in ATTACHMENT_RAW_PATTERN.findall(raw_content)]
        url_sha1s.extend((urljoin(Shuiyuan_Base, url), sha1) for url, sha1 in zip(cooked_match, raw_match))
    return url_sha1s
...
def match_replace(path: str, filename: str, topic: str):
    ...
    for url, sha1_with_ext in _collect_attachment_links(topic):
