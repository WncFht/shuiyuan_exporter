...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Ensure a Shuiyuan topic exists in the skill cache."
    )
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force sync even if topic looks unchanged"
    )
...
def main() -> int:
    ...
    print_json(
        api.ensure_topic_cached(
            ...
            force=args.force,
            progress_callback=build_progress_reporter("ensure_cached"),
        )
    )
    return 0
