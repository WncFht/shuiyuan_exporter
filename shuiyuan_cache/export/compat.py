...
def resolve_auth_cookie_header(path: str = _cookie_default_path, cache_root: str = _default_cache_root) -> str:
    config = CacheConfig(cache_root=Path(cache_root), cookie_path=Path(path))
    storage_cookie = build_cookie_header_from_storage_state(config.storage_state_path, config.base_url)
    if storage_cookie:
