...
DELETED_IMAGE_PATTERN = re.compile(
    r'<img src="[a-zA-Z0-9_:\/\-\.]+" alt=".*?" data-orig-src="(.*?)"'
)
IMAGE_PATTERN = re.compile(
    r'<img src="([a-zA-Z0-9_:\/\-\.]+)" alt=".*?" data-base62-sha1="(.*?)"'
)
...
def _collect_image_rewrites(
    ...
    for post in cache_bridge.iter_json_posts(topic_id):
        ...
        deleted_names.extend(
            name.removeprefix("upload://")
            for name in DELETED_IMAGE_PATTERN.findall(html_content)
        )
